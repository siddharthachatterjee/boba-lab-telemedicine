...
let TRANSFERABILITY_TEXT = "";
if (MODE == 1) {
    TRANSFERABILITY_TEXT = "Your Telemedicine cases will feature a different type of patient.";
}
else {
    TRANSFERABILITY_TEXT="Telemedicine cases match your hospital patients.";
}
const PHASE_CONFIG = {
...
class SideJob {
    constructor(config) {
        ...
        this.startSceneMap = config.startSceneMap || null;
        this.maxReward = config.maxReward;  // map returnScene → startScene
    }
}
...
class MainJobGameInstance extends GameInstance {
    constructor(config) {
        ...
        this.skipAdvanceHandler = (e) => this.handleSkipAdvanceKey(e);
        this.activeUberSkip = null;
    }
    ...
    handleSkipAdvanceKey(e) {
        ...
        if (!scene) {
            const fallbackBtn = document.querySelector('#choices button');
            if (fallbackBtn) fallbackBtn.click();
            return;
        }

        if (this.onBreak || /^startBreak/.test(this.currentScene)) {
            this.displayScene('shiftTransition');
            return;
        }

        if (scene.actionItems) {
            const uberGameItem = scene.actionItems.find(item => item.type === 'uberGame');
            if (uberGameItem) {
                if (this.activeUberSkip) {
                    this.activeUberSkip();
                } else {
                    this.displayScene(uberGameItem.nextScene || '__return__');
                }
                return;
            }

            const sliderItem = scene.actionItems.find(item => item.type === 'slider');
            if (sliderItem) {
                const storyEl = document.getElementById('scene-story');
                if (storyEl) storyEl.textContent = 'Correct! The value is ' + sliderItem.correctValue + '.';
                this.onSliderSubmit(true, sliderItem.correctValue);
                this.displayScene(sliderItem.nextScene);
                return;
            }

            const teleLoginItem = scene.actionItems.find(item => item.type === 'teleLogin');
            if (teleLoginItem) {
                this.displayScene(teleLoginItem.nextScene);
                return;
            }
        }

        if (scene.choices && scene.choices.length > 0) {
            const correctChoice = scene.choices.find(choice =>
                choice.correct === true ||
                (typeof choice.next === 'string' && choice.next.toLowerCase().includes('correct')) ||
                (typeof choice.text === 'string' && choice.text.toLowerCase().includes('(correct)'))
            );
            if (correctChoice) {
                this.onChoiceMade(this.currentScene, correctChoice.next, correctChoice);
                this.displayScene(correctChoice.next);
                return;
            }

            if (this.onSideJob && scene.choices.length > 0) {
                const finalChoice = scene.choices[scene.choices.length - 1];
                this.onChoiceMade(this.currentScene, finalChoice.next, finalChoice);
                this.displayScene(finalChoice.next);
                return;
            }
        }

        if (this.onSideJob) {
            this.displayScene('__return__');
            return;
        }
        ...
        if (scene.choices && scene.choices.length === 1) {
            const onlyChoice = scene.choices[0];
            this.onChoiceMade(this.currentScene, onlyChoice.next, onlyChoice);
            this.displayScene(onlyChoice.next);
            return;
        }
    ...
    startUberGame(item) {
        ...
        this.activeUberSkip = () => {
            ...
            this.activeUberSkip = null;
            this.displayScene(item.nextScene || '__return__');
        };
    ...
    onChoiceMade(fromKey, toKey, choice) {
        ...
        const normalizedToKey = typeof toKey === 'string' ? toKey.toLowerCase() : '';
        if (normalizedToKey.includes('correct') || normalizedToKey.includes('wrong')) {
            this.totalAttempts++;
            const isCorrect = normalizedToKey.includes('correct');
            let rewardDelta = 0;
            if (isCorrect) {
                this.totalCorrect++;
                if (this.onSideJob) {
    ...
    displayScene(sceneKey) {
        ...
        if (sceneKey === '__phaseEnd__') {
            ...
            continueBtn.onclick = () => {
                ...
                if (this.onPhaseEnd) {
                    ...
                    if (this.Q) this.Q.showNextButton();
                }
            };
        ...
        if (scene.choices && scene.choices.some(c => c.hasOwnProperty('correct'))) {
            ...
            const skipChoice = /^(patient|alienPatient)/.test(sceneKey)
                ? scene.choices.find(choice => choice.skip)
                : null;
            const shuffled = skipChoice
                ? scene.choices.filter(choice => !choice.skip)
                : scene.choices.slice();
            for (let i = shuffled.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                const tmp = shuffled[i]; shuffled[i] = shuffled[j]; shuffled[j] = tmp;
            }
            scene.choices = skipChoice ? shuffled.concat(skipChoice) : shuffled;
        }
    ...
    showBreakChoice(breakItem) {
        ...
        sorted.forEach(job => {
            ...
            btn.textContent = job.name +
                ' — Reward Rate: ' + job.reward +
                ' | Trains: ' + job.actionItem;
            if (job.maxReward > 0) {
                btn.textContent += ' | Max Reward: ' + job.maxReward;
            }
            btn.onclick = () => {
                cleanup();
    ...
    runSideJobAction(job, returnScene, sceneLookupKey) {
        ...
        let startScene;
        if (job.id === 'telemedicine' && this.phaseId === 1) {
            startScene = TRANSFERABILITY_SCENES.low[Math.floor(Math.random() * TRANSFERABILITY_SCENES.low.length)];
        } else if (job.id === 'telemedicine' && this.phaseId === 3 && this.transferabilityMode !== null) {
    ...
    stopTimer() {
        ...
        clearTimeout(this.inactivityTimeout);
        clearInterval(this._inactivityCountdownInterval);
        document.removeEventListener('keydown', this.skipAdvanceHandler);
        this.activeUberSkip = null;
    }
    ...
    start() {
        ...
        document.removeEventListener('keydown', this.skipAdvanceHandler);
        document.addEventListener('keydown', this.skipAdvanceHandler);

        // Assign transferability mode for Phase 3 only
        if (this.phaseId === 3) {
            this.transferabilityMode = MODE;
        } else {
...
const TELEMEDICINE_SCENES = {
    ...
    scenes: {
        ...
        phase3Intro: {
            ...
            story: "Welcome to Phase 3. You will see a fresh set of patients over another 5-day week. Side jobs are available during breaks as before. " + TRANSFERABILITY_TEXT,
